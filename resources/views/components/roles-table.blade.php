    ...
    <table class="w-full border-collapse border rounded-xl border-blue-400">
        ...
        @forelse ($roles as $key => $role)
            <tr class="hover:bg-gray-50 transition-colors h-14">
                ...
                <td class="">
                    <div class="ml-5 border rounded-full h-7 w-7" style="background-color: {{ $role->color }}"></div>

                </td>

                <td class="px-4 py-2">{{ Carbon\Carbon::parse($role->created_at)->format('d-m-Y') }}</td>
                <td class="px-4 py-2">{{ Carbon\Carbon::parse($role->updated_at)->format('d-m-Y') }}</td>
